...
$disciplines = [''=>[],'FS'=>[1,2,3],'AE'=>[4],'IS'=>[5,6],'DY'=>[10,11],'WS'=>[7,12],'CP'=>[8],'CF'=>[9],'SP'=>[13]]; 

//2. Проверяем фильтры
$F_Year = ( isset($_COOKIE["Year"]) && filter_var($_COOKIE["Year"],FILTER_VALIDATE_INT)) ? intval($_COOKIE["Year"]) : 0;
$F_Type = ( isset($_COOKIE["Type"]) && array_key_exists($_COOKIE["Type"],$disciplines)) ? $_COOKIE["Type"] : '';
...
/*	.(($F_Name != '') ? " AND (DESCRIPTION->'$.name' LIKE '%'||'$F_Name'||'%') OR (DESCRIPTION->'$.location' LIKE '%'||'$F_Name'||'%')" : '')
	...
	.(($F_Type != '') ? " AND DESCRIPTION->$.type_id IN (".implode(',',$disciplines[$F_Type]).")" : '')
*/
//MariaDB
    ...
    .(($F_Type != '') ? " AND JSON_VALUE(DESCRIPTION, '$.type_id') IN (".implode(',',$disciplines[$F_Type]).")" : '')
